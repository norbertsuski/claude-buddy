This project uses the Contributor Covenant, version 2.1, reproduced below word
for word. It is a well-known text on purpose: a code of conduct is only useful
if a contributor already knows what they are agreeing to before they read it,
and a bespoke one written for one small project buys nothing but the chance of
saying something unintended. The only thing changed from the canonical version
is the reporting contact in *Enforcement*, which the covenant leaves as a blank
for each project to fill in.
...
most cannot — open an issue saying only that you have a conduct report to make,
naming nobody and describing nothing, and wait to be contacted. That first
issue is a request for a private conversation, not the report itself, and it

...
Hover anywhere on the black and that same element grows — down and out to a third of the display's width — into a list of every session with its status and elapsed time, and the detail of the row under the cursor opens beneath it. There is no popover in this mode, and none is needed: the detail carries the popover's own fields — what the session is doing, the background tasks it is waiting on, its name, working directory, branch, model and process — leaving out only the three the row and the footer already say. Clicking the row raises that session's window, the same as clicking a popover. A tasking session's detail names what it is waiting on — the task's own name — the same way a waiting session's detail names its reason. Leave it and it collapses back into the menu bar.
...
Background work is listed in the detail of the session that owns it rather than beside it — four agents rendered as four more rows buried the three sessions they belonged to. That list is the session's own tasks, so it says what each one is and how long it has been going, and it is there whether or not *Show background jobs* gives a registry job its own row.

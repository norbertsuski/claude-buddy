    ...
    std::thread::spawn(|| {
        ...
        loop {
            ...
            if crate::config::cached().show_usage && !fixture_mode() {
                if let Some(usage) = fetch(crate::watcher::watch::now_ms()) {
...
fn token_from_json(bytes: &[u8]) -> Option<String> {
    ...
    if let Some(expires_at) = oauth.get("expiresAt").and_then(|v| v.as_i64()) {
        if expires_at <= crate::watcher::watch::now_ms() {
            return None;
        }
    }
...
mod tests {
    ...
    fn future_ms() -> i64 {
        crate::watcher::watch::now_ms() + 3_600_000
    }
    ...
    fn a_response_body_parses_as_a_utilization_object() {
        let now = crate::watcher::watch::now_ms();
